...
const content = [
    {
        ...
        date: 'Released on March 2022',
        summary: 'Website and App Mobile for Booking Stylist',
        link: [
            {
                name: 'Source Code',
                url: 'github.com'
...
const oUEH: React.FC<inUEH> = ({theme}) => {
    ...
    return (
        <section className="grid justify-items-center">
            <div className="w-3/5 grid grid-cols-5 gap-4">
                ...
                <motion.div
                    className="col-span-3 h-fit-content bg-violet-600/25 rounded-lg"
                    initial={{
                        opacity: 0,
                        translateX: 800
                    }}
                ...
                >
                    <ol className="relative border-s border-gray-200 dark:border-gray-700 mt-5 mb-5">
                            ...
                            content.map((item, index) => {
                                return (
                                    <li key={index} className="mb-10 ms-6">
                                        ...
                                        <h3 className="flex items-center mb-1 text-lg font-semibold text-gray-900 dark:text-white">{ item.name }<span className="bg-blue-100 text-blue-800 text-sm font-medium me-2 px-2.5 py-0.5 rounded dark:bg-blue-900 dark:text-blue-300 ms-3">Latest</span></h3>
                                        <time className="block mb-2 text-sm font-normal leading-none text-gray-400 dark:text-gray-500">{ item.date }</time>
                                        <p className="mb-4 text-base font-normal text-gray-500 dark:text-gray-400">{ item.summary }</p>
                                        {
                                            item.link.map((item, index) => {
                                                return (
                                                    <a key={index} href={item.url} target="_blank"
                                                        className="inline-flex items-center px-4 py-2 text-sm font-medium mr-1
                                                                 text-gray-900 bg-white border border-gray-200 rounded-lg hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-4 focus:outline-none focus:ring-gray-200 focus:text-blue-700
